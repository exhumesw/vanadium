...
#define MAATTACHEDWINDOW_DEFAULT_INNER_BORDER_COLOR [NSColor blackColor]
                        ...
                        atDistance:(float)distance
{
    ...
    if ((self = [super initWithContentRect:contentRect 
        ...
        innerBorderColor = [MAATTACHEDWINDOW_DEFAULT_INNER_BORDER_COLOR copy];
        borderWidth = 2.0;
...
- (NSColor *)_backgroundColorPatternImage
{
	...
	if (innerBorderWidth > 0) {
        ...
        [bgPath setLineWidth:((borderWidth * 2.0) + (innerBorderWidth * 2.0)) * MAATTACHEDWINDOW_SCALE_FACTOR];
...
- (void)setInnerBorderColor:(NSColor *)value {
    if (innerBorderColor != value) {
        innerBorderColor = [value copy];
...
- (void)setInnerBorderWidth:(float)value {
    if (innerBorderWidth != value) {
        ...
        if (value <= maxBorderWidth) {
            ...
            innerBorderWidth = maxBorderWidth;
        }
